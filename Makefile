...
.PHONY: docker-build docker-run docker-stop docker-clean run test

docker-build:
	@echo "Copying FARD source (excluding target/)..."
	@rm -rf fard_src
	@rsync -a --exclude=target/ --exclude=.git/ $(FARD_SRC)/ fard_src/
	@echo "Building Docker image..."
	docker build -t $(IMAGE_NAME):$(IMAGE_TAG) .
	@rm -rf fard_src
	@touch fard_src/.gitkeep
